...
<?php print $wrapper_prefix; ?>
  <div id="showResults" class="search-header">
	...
	<?php  
	   ...
	   if ($_GET['title'] == "" && $_GET['field_cuisine_term_tid']=="All" && is_array($_GET["field_culture_offer_value"])==false 
    ...
    <p class="floatRight"><?php print $display_search_for ?>  
	  ...
	  if($var_alternative==false){
			...
			if ($_GET['field_cuisine_term_tid'] != "") {
			  if (!$first) {
				 if(count($rows)>1){
				  print " and ";
				}
			  }
			  $term = taxonomy_term_load(htmlspecialchars($_GET['field_cuisine_term_tid']));
