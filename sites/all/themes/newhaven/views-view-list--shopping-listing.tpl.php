...
<?php print $wrapper_prefix; ?>
  <div id="showResults" class="search-header">
	...
	<?php   
	   ...
	   if ($_GET['title'] == "" && $_GET['field_shopping_category_value']=="All" && is_array($_GET["field_culture_offer_value"])==false 
  ...
  <p class="floatRight"><?php print $display_search_for ?>  
	  ...
	  if($var_alternative==false){
			...
			if ($_GET['field_shopping_category_value'] != "") {
			  if (!$first) {
				 if(count($rows)>1){
				  print " and ";
				}
			  }
			  print "<span class='filter'>" . ucwords($_GET['field_shopping_category_value']) . "</span>";
			}
			 ...
			 if ($_GET["q"] != "node/13") {
			  ...
			  print "<span class='filter'>" . strtoupper($letter) . "</span>";
			   ...
			   if ($_GET['field_shopping_category_value'] == "" && $_GET['field_price_point_value'] == "" && $_GET['field_neighborhood_value'] == ""){
